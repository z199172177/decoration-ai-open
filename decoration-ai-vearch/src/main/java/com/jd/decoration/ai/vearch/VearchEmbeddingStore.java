...
import com.jd.decoration.ai.vearch.request.DocumentSearchRequest;
import com.jd.decoration.ai.vearch.request.DocumentUpsertRequest;
import com.jd.decoration.ai.vearch.request.VectorSearchRequest;
import com.jd.decoration.ai.vearch.response.DocumentSearchResponse;
...
public class VearchEmbeddingStore implements EmbeddingStore<TextSegment> {
    ...
    public List<EmbeddingMatch<TextSegment>> findRelevant(Embedding referenceEmbedding, int maxResults, double minScore) {
        return vectorSearch(referenceEmbedding, maxResults, minScore);
    }

    /**
     * @deprecated see documentSearch
     */
    @Deprecated
    @NotNull
    private List<EmbeddingMatch<TextSegment>> vectorSearch(Embedding referenceEmbedding, int maxResults, double minScore) {
        ...
        JsonObject result = vearchClient.vectorSearch(this.database, this.space, searchRequest);
        VectorSearchResponse searchResponse = GSON.fromJson(result, VectorSearchResponse.class);
        if (searchResponse == null || searchResponse.getHits() == null || searchResponse.getHits().getTotal() == null) {
            return new ArrayList<>();
        }

        List<VectorSearchResponse.HitDetail> retDocumentHitList = searchResponse.getHits().getHits();
        if (CollectionUtil.isEmpty(retDocumentHitList)) {
            return new ArrayList<>();
        }

        return retDocumentHitList.stream()
                .map(item ->  {
                    String id = item.getId();
                    Double score = item.getScore();
                    TextSegment textSegment = null;
                    Object textObj =  item.getSource().get(this.textField);
    ...
    @NotNull
    private List<EmbeddingMatch<TextSegment>> documentSearch(Embedding referenceEmbedding, int maxResults, double minScore) {
    ...
    private void addAllInternal(List<String> ids, List<Embedding> embeddings, List<TextSegment> textSegments) {
        ...
        documentAdd(ids, embeddings, textSegments);

    }

    /**
     * @deprecated see documentUpsert
     */
    @Deprecated
    private void documentAdd(List<String> ids, List<Embedding> embeddings, List<TextSegment> textSegments) {
        for (int i = 0; i < ids.size(); ++i) {
            ...
            bodyMap.put(this.vectorField, vectorMap);

            if (textSegments != null && textSegments.get(i) != null) {
                TextSegment textSegment = textSegments.get(i);
                bodyMap.put(this.textField, textSegment.text());
            }

            vearchClient.documentAdd(this.database, this.space, id, bodyMap);
        }

    }

    private void documentUpsert(List<String> ids, List<Embedding> embeddings, List<TextSegment> textSegments) {
